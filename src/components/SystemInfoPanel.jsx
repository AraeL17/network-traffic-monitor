...
export default function SystemInfoPanel({ info }) {
  ...
  return (
    <section className="glass-panel p-4 shadow-panel">
      <h2 className="mb-4 text-sm font-bold text-slate-950">当前电脑系统配置</h2>
...
function Metric({ label, value }) {
  return (
    <div className="mt-4">
      <div className="mb-2 flex justify-between text-xs text-slate-400"><span>{label}</span><span>{value}%</span></div>
      <div className="h-2 rounded-full bg-slate-200/70"><div className="h-full rounded-full bg-gradient-to-r from-cyan-neon to-matrix-bright" style={{ width: `${value}%` }} /></div>
    </div>
  )
}
