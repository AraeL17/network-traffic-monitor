...
export default function PacketDetailDrawer({ packet, unit, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-900/25 backdrop-blur-sm" onClick={onClose}>
      <aside className="ml-auto h-full w-full max-w-xl border-l border-white/70 bg-white/72 p-6 shadow-neon-cyan backdrop-blur-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-6 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-lg border border-slate-200 bg-white/55 p-2 text-slate-500 shadow-sm hover:text-cyan-neon"><X className="h-5 w-5" /></button>
        </div>
        <div className="space-y-4">
          ...
          <Panel title="风险说明">
            ...
            <p className="text-sm text-slate-400">{packet.risk === 'high' ? '该数据包命中高风险模拟规则，可能涉及异常端口或跨境大流量。' : packet.risk === 'suspicious' ? '该数据包存在可疑特征，建议结合真实后端规则进一步确认。' : '当前模拟规则未发现明显风险。'}</p>
          </Panel>
        </div>
      </aside>
    </div>
...
function Panel({ title, children }) {
  return <section className="rounded-lg border border-slate-200/80 bg-white/52 p-4 shadow-sm"><h3 className="mb-3 text-sm font-bold text-slate-950">{title}</h3>{children}</section>
}
