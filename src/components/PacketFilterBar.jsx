  ...
  return (
    <section className="glass-panel p-4 shadow-panel">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="console-panel-title flex items-center gap-2"><Filter className="h-4 w-4 text-cyan-neon" />数据包筛选</h2>
        <button onClick={() => { setDraftFilters(defaultFilters); onReset() }} className="flex items-center gap-1 text-xs text-slate-500 hover:text-cyan-neon"><RotateCcw className="h-3.5 w-3.5" />重置筛选</button>
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={onApply} className="rounded bg-cyan-neon px-5 py-2 text-sm font-black text-white shadow-neon-cyan">应用筛选</button>
      </div>
    </section>
  )
}
...
function Input({ label, value, onChange, placeholder, type = 'text' }) {
  return (
    <label className="block">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="console-input text-sm placeholder:text-slate-600" />
    </label>
  )
}
...
function Select({ label, value, onChange, options, compact = false }) {
  return (
    <label className={`block ${compact ? 'w-48' : ''}`}>
      <span className="mb-1 block text-xs text-slate-500">{label}</span>
      <select value={value} onChange={(e) => onChange(e.target.value)} className="console-input text-sm">
